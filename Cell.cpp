...
Cell::Cell(Satellite * satellites, RocketBody * rockets, Array2D<double> & N_i, size_t num_sat_types,
    ...
    */
    ...
    this->lethal_sat_N = new vector<Array2D<bool> *>();
    this->lethal_rb_N = new vector<Array2D<bool> *>();
    ...
    for (size_t i = 0; i < this->num_sat_types; i++) {
        (this->lethal_sat_N)->push_back(Array2D<bool>::fill_dyn(true, this->num_L, this->num_chi));
        if (this->target_alt->get(i, 0) > (this->alt + (this->dh)/2)) {(this->trackable)->set(true, i, 0);}
    }
    for (size_t i = 0; i < this->num_rb_types; i++) {
        (this->lethal_rb_N)->push_back(Array2D<bool>::fill_dyn(true, this->num_L, this->num_chi));
    }
...
Cell::~Cell() {
    ...
    //        will not free them
    delete this->trackable; delete [] this->ascending; delete this->lethal_rb_N;
    delete this->lethal_sat_N; delete this->tau_N; delete [] this->event_list;
    ...
    delete this->lam_s; delete this->sigma_s; delete this->m_s;
}