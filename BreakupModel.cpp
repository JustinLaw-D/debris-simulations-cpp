...
double calc_M(double m_t, double m_d, double v) {
    ...
    */
    ...
    double E_p = (0.5*m_d*((v*1000)*(v*1000))/m_t)/1000; // E_p in J/g

    if (E_p >= 40) {return m_t + m_d;} // catestrophic collision
...
double calc_Ntot(double M, double Lmin, double Lmax, char typ, double C) {
    ...
    */
    ...
    if (typ == 'c') {
        ...
        return 6*C*(pow(Lmin,-1.6) - pow(Lmax,-1.6));
    }
}
...
double X_cdf(double x, double x_min, double x_max, double L, char typ) {
    ...
    */
    ...
    else if (L <= 8/100) {return _X_cdf_8(x, x_min, x_max, L);}
    else {
        double lam_min = log10(8/100);
        double lam_max = log10(11/100);
...
double _X_cdf_8(double x, double x_min, double x_max, double L) {
    ...
    */
    ...
    double C = 1/(erf((x_max-mu)/(SQRT2*sigma)) - erf((x_min-mu)/(SQRT2*sigma)));
...
double _X_cdf_11(double x, double x_min, double x_max, double L, char typ) {
    ...
    */
    ...
    if (typ == 's') {
        ...
        if (lam <= -1.1) {mu1 = -0.6;}
        else if (lam < 0) {mu1 = -0.6 - 0.318*(lam + 1.1);}
        else {mu1 = -0.95;}
        ...
        if (lam <= -1.4) {alpha = 1;}
        else if (lam < 0) {alpha = 1 - 0.3571*(lam + 1.4);}
        else {alpha = 0.5;}

        if (lam <= -0.5) {mu1 = -0.45;}
        else if (lam < 0) {mu1 = -0.45 - 0.9*(lam + 0.5);}
        else {mu1 = -0.9;}

        sigma1 = 0.55; mu2 = -0.9;

        if (lam <= -1) {sigma2 = 0.28;}
        else if (lam < 0.1) {sigma2 = 0.28 - 0.1636*(lam + 1);}
        else {sigma2 = 0.1;}
    }   
    
    // compute normalization factor
    double top = alpha*erf((x_max-mu1)/(SQRT2*sigma1)) + (1-alpha)*erf((x_max-mu2)/(SQRT2*sigma2));
    double bot = alpha*erf((x_min-mu1)/(SQRT2*sigma1)) + (1-alpha)*erf((x_min-mu2)/(SQRT2*sigma2));
    double C = 1/(top - bot);
    // compute total distribution
...
double v_cdf(double v, double x, char typ) {
    ...
    */
    ...
    // calculate CDF value
    return 0.5*(erf((v-mu)/sigma_fac) + 1);
}
...
double vprime_cdf(double V, double v0, double theta, double phi, double x, char typ) {
    ...
    */
    ...
    double descriminate = pow((v0*sin(theta)*cos(phi)), 2) - (v0*v0-V*V);
    ...
    else if (del_v_min <= 0) {return v_cdf(log10(del_v_max), x, typ);}
